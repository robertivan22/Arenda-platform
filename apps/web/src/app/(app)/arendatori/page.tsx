    ...
    <div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 && <tr><td colSpan={6} className="px-3 py-8 text-center text-gray-400">Niciun arendator inregistrat</td></tr>}
            {filtered.map(r => (
              <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/arendatori/${r.id}/sumar`)}>
                <td className="px-3 py-2 font-mono text-xs text-gray-500">{r.code}</td>
                <td className="px-3 py-2 font-medium">{displayName(r)}</td>
