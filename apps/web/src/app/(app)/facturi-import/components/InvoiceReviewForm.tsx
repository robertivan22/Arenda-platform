...
export function InvoiceReviewForm({ invoice, onUpdated, onApproved }: Props) {
  ...
  const approve = async () => {
    ...
    try {
      ...
      if (!userId) throw new Error('Sesiune expirată')
      ...
      const { data, error } = await db
        ...
        .single()
      if (error) throw new Error(error.message)
      toast.success('Factura confirmată și stocul actualizat!')
      onUpdated(data as unknown as ImportRecord)
      onApproved()
    } catch (e) {
      toast.error((e as Error).message)
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {invoice.status === 'failed' && (
          <span className="text-sm text-red-600">{invoice.error_message ?? 'OCR eșuat'}</span>
        )}
      </div>
      ...
      {isPosted && (
        <div className="flex items-center gap-2 text-green-700 bg-green-50 border border-green-200 rounded-lg px-4 py-3">
          ...
          <span className="font-medium">Factura confirmată. Stocul actualizat.</span>
        </div>
      )}
    </div>
  )
