...
interface Props {
  ...
  refreshKey?: number
  currentDistributionKg?: number
  onCropBreakdown?: (b: { contractedByCrop: Record<string, number>; distributedByCrop: Record<string, number> } | null) => void
}

export function SumarArendator({ landlord, refreshKey, currentDistributionKg = 0, onCropBreakdown }: Props) {
  ...
  useEffect(() => {
    onCropBreakdown?.(cropBreakdown)
  }, [cropBreakdown, onCropBreakdown])
              ...
              {Object.entries(cropBreakdown.contractedByCrop).map(([crop, total]) => {
                ...
                return (
                  <tr key={crop} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className={clsx('py-1.5 text-xs text-right font-semibold', remaining === 0 ? 'text-gray-400 line-through' : 'text-amber-700')}>
